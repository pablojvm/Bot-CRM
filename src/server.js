...
// ==========================================================================
// Meta Leads processor (cron) - SOLO lock + marcar (placeholder)
// ==========================================================================

async function runMetaLeadsProcessor() {
    const clientId = "default";
    ...
    for (const item of rows) {
        // 2) lock optimista (evita doble procesado si hay 2 crons)
        const lock = await pool.query(
            `
      update leads_inbox
      ...
      `,
        ...
        // 3) TODO: aquí luego haremos:
        // - extraer leadgen_id
        // - llamar a Graph API
        // - upsert en leads
        // - trigger WhatsApp / email / agenda

        // por ahora lo dejamos en processed para ver que corre el cron
        await pool.query(
            `
      update leads_inbox
...
app.get("/cron/meta-leads", async (req, res) => {
    try {
        const token = req.query.token;
        if (!token || token !== process.env.CRON_TOKEN) return res.status(401).send("unauthorized");

        const result = await runMetaLeadsProcessor();
        res.json({ ok: true, ...result });
    } catch (e) {
        console.error("CRON META LEADS ERROR ❌", e);
        res.status(500).json({ ok: false });
    }
});
